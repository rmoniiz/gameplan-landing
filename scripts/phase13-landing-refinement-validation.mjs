...
check('runtime removes duplicate tactical journey', runtime.includes("document.querySelector('.tactical-journey')") && runtime.includes('?.remove()'));
check('runtime uses four straight coach-like tactical routes', (runtime.match(/data-route=/g) || []).length === 4 && runtime.includes('hero-tactic-arrow'));
check('runtime avoids Bezier curves in hero route definitions', !/data-route=[\s\S]{0,140}\bd="[^"]*\bC\b/.test(runtime));
check('runtime provides six football-specific module visuals', (runtime.match(/data-visual=/g) || []).length === 6);
check('runtime gives game model a collective structure visual', runtime.includes('data-visual="game-model-structure"') && runtime.includes("'ATAQUE'") && runtime.includes("'DEFESA'"));
check('runtime gives planning a weekly agenda visual', runtime.includes('data-visual="weekly-plan"') && runtime.includes("'SEG'") && runtime.includes("'DOM'"));
check('runtime gives exercise card a training-task visual', runtime.includes('data-visual="exercise-task"'));
check('runtime gives development a player-context visual', runtime.includes('data-visual="player-development"'));
check('runtime gives tactical test a decision visual', runtime.includes('data-visual="tactical-decision"'));
check('runtime gives match statistics a pitch-events visual', runtime.includes('data-visual="match-events"'));
check('runtime lowers initial demo volume to 8 percent', runtime.includes('demoVideo.volume = 0.08'));
check('runtime keeps video controls untouched', !runtime.includes("removeAttribute('controls')") && !runtime.includes('controls = false'));
check('runtime uses approved PT-BR About title', runtime.includes('O GamePlan nasceu de uma necessidade.'));
check('runtime uses independent EN About title', runtime.includes('GamePlan started with a need.'));
check('runtime removes disliked friction wording', !runtime.includes('fricção'));
check('runtime preserves personal football origin', runtime.includes('Minha relação com o futebol começou com meu avô') && runtime.includes('My relationship with football began with my grandfather'));
check('runtime preserves coach decision principle', runtime.includes('decisão técnica continuam com o treinador') && runtime.includes('technical decisions stay with the coach'));
check('runtime uses approved final CTA PT-BR', runtime.includes('Leve sua semana para dentro do GamePlan.'));
check('runtime uses natural final CTA EN', runtime.includes('Bring your week into GamePlan.'));
